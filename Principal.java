...
public class Principal {
	...
	/**
	 * Permite conectarse con la base de datos utilizando los parÃ¡metros globales de la clase.
	 * AÃ±ade el objeto conexiÃ³n como variable de la clase para que todos los mÃ©todos puedan acceder.
	 */
	public static boolean conectar()
	 {
		...
		if (conn != null)
		 {
			System.out.println("ConexiÃ³n correcta con la base de datos: "+DB+".");
		 }
		else
		 {
	...
	/**
	 * Este mÃ©todo realiza una bÃºsqueda de espectÃ¡culos y los lista.
	 * @param Nombre -> Nombre del espectÃ¡culo (En caso de pasar una cadena vacia, busca uno cualquiera).
	 * @param tipo -> Tipo de espectÃ¡culo (En caso de pasar una cadena vacia, busca uno cualquiera).
	 * @return -> Lista de espectÃ¡culos encontrados.
	 */
	...
	/**
	 * Busca los eventos existentes que coincidan con los parÃ¡metros que se le pasan.
	 * @param ID_espectaculo -> Identificador del espectÃ¡culo al que corresponde el evento (Si es 0, se busca cualquiera).
	 * @param ciudad -> Ciudad donde se celebra el evento (En caso de pasar una cadena vacia, busca uno cualquiera).
	 * @param id_recinto -> Identificador del recinto donde se celebra el espectÃ¡culo (Si es 0, se busca cualquiera).
	 ...
	 * Disponibles: Solo lista las localidades  que no estÃ¡n ocupadas para ese evento, que estÃ¡n en una grada asignada para ese evento y que no estÃ©n defectuosas.
	 * AdemÃ¡s, en caso de encontrar una grada pre-reservada pero que su plazo de como pre-reserva estÃ© espirado, esta tambiÃ©n se mostrarÃ¡ como libre para tener la posivilidad de reservarla.
	 ...
	 * 		-1  -> El DNI introducido no existe para ningÃºn cliente-
	 *		-2  -> No existe el evento introducido.
	 *		-3  -> No existe la reserva introducida.
	 *		-4  -> No se pueden aÃ±adir entrada a una reserva (estÃ¡ pagada)
	 *		-5  -> Error de SQL.
     *		-11 -> La localidad estÃ¡ ocupada.
	 *		-12 -> La grada donde se situa esta localidad no estÃ¡ disponible en este evento para el usuario indicado.
	 *		-13 -> La localidad estÃ¡ deteriorada.
			...
			for(Entry<Integer, String> l_u : Localidad_Usuario.entrySet())
			 {
				...
				cstmt.setInt(7, 0);//Esto anula la transacciÃ³n del propio procedure y solo utiliza la transacciÃ³n de la funciÃ³n java.
	...
	/**
	 * Esta funciÃ³n permite realizar el pago de una reserva del cliente con el DNI introducido en el sistema.
...
/**
	 * Permite anular una reserva. En caso de no estar pagada (pre-reserva), simplemente se eliminarÃ¡ la reserva con sus correspondientes entradas.
	 * Si la reserva ya ha sido pagada se le devolverÃ¡ el importe total de la reserva, siempre que no pase del periodo de validez,
	 * en este caso el importe que se le devolverÃ¡ serÃ¡ una parte propocional del total dependiente del evento.
	 * @param ID_reserva -> Identificador de la reserva que se desea anular.
	 * @return
	 * 		    1 -> Correcto. La reserva se ha anulado corrrectamente. (Se devuelve el importe Ã­ntegro de la reserva, no ha superado la fecha de validez)
	 ...
	 * @param ID_localidad_nueva -> Identificador de la localidad nueva a la que se quiere modificar (En caso de 0, no se modificarÃ¡)
	 * @param Tipo_usuario_nuevo -> Tipo de usuario por el que se quiere modificar la entrada (En caso de cadena vacia, no se modificarÃ¡)
	 * @return  1 -> ModificaciÃ³n de la entrada realizada correctamente.
	 * 		   -1 -> La entrada no existe
	 *		   -2 -> No se pueden modificar entradas de una reserva (estÃ¡ pagada)
	 *        -11 -> La localidad estÃ¡ ocupada.
	 *		  -12 -> La grada donde se situa esta localidad no estÃ¡ disponible en este evento para el usuario indicado.
	 *		  -13 -> La localidad estÃ¡ deteriorada.
	 *		  -14 -> La localidad no existe para esta grada.
	...
	int verEspectaculos(BufferedReader reader){
		...
		print("");
		try{
			print("Introduzca numero de espectáculo para ver eventos de ese espectáculo (enter para salir)");
			String temp = null;
	...
	void menuAnularReserva(int id){
		...
		switch(resultado){
			case 1: print(" La reserva se ha anulado corrrectamente. (Se devuelve el importe íntegro)");
				break;
			case 2: print(" La reserva se ha anulado corrrectamente. (Se devuelve un porcentaje del importe débido a superar la fecha de validez)");
				break;
